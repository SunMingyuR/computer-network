...
int main(int arg, char *argv[])
{
    ...
    while(1)
    {
        ...
        //write函数同理
        if(read(clientSocket, command, N) < 0)  //read函数从cli_sockfd中读取N个字节数据放入commd中
        {
            printf("Read Error!\n");
            exit(1);
        }
        ...
        }else if(strncmp(command,"download", 3) == 0 )
        {
            commandDownload(clientSocket, command+4);
        }else if(strncmp(command, "put", 3) == 0)
        {
            commandUploadt(clientSocket, command+4);
        }else
        {
